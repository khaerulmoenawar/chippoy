...
function getFastPosition(element) {
...
function simpleDelay(func, wait) {
  let timeout;
  return function () {
...
function checkVisibility() {
  ...
  const elements = document.querySelectorAll("section, .note, .promise");

  elements.forEach((element) => {
    const rect = getFastPosition(element);
...
window.addEventListener("resize", () => rectCache.clear());

window.addEventListener("scroll", simpleDelay(checkVisibility, 50));
...
function initCustomCursor() {
  ...
  const interactiveElements = document.querySelectorAll(
    "a, button, .gallery-item, .love-lock, .note, .promise, .memory-content",
  );
...
function initLoveLock() {
  ...
  function createFloatingHeart(lockElement) {
    ...
    heart.style.color = getComputedStyle(
  ...
  function addSparkleEffect(lockElement) {
    for (let i = 0; i < 10; i++) {
      setTimeout(() => {
        ...
        sparkle.style.left =
          getCachedRect(lockElement).left + Math.random() * 100 + "px";
        sparkle.style.top =
          getCachedRect(lockElement).top + Math.random() * 100 + "px";
...
function initMusicPlayer() {
  ...
  audio.play().catch((e) => {
    ...
    musicPlayer.querySelector(".music-info").textContent =
      "November the 2nd - Click to play";
  });
  ...
  playPauseBtn.addEventListener("click", () => {
    ...
    if (isPlaying) {
      ...
      musicPlayer.querySelector(".music-info").textContent =
        "November the 2nd - Playing";
    } else {
      audio.pause();
      playPauseBtn.innerHTML = '<i class="fas fa-play"></i>';
      musicPlayer.querySelector(".music-info").textContent =
        "November the 2nd - Paused";
    }
  });
}
  ...
  memoryTriggers.forEach((trigger) => {
    trigger.addEventListener("click", () => {
      ...
      if (memory) {
        ...
        document.getElementById("memoryPopupContent").innerHTML =
          memory.content;
...
function updateCountdown() {
  ...
  console.log(
    `Time elapsed: ${years} years, ${months} months, ${days} days, ${hours} hours, ${minutes} minutes, ${seconds} seconds`,
  );
}
...
function initGalleryAndLightbox() {
  ...
  function populatePopupGallery(key, count = 6) {
    ...
    const assets =
      (galleryAssets[key] && galleryAssets[key].slice()) ||
      galleryAssets["all-memories"].slice() ||
      [];
  ...
  function attachPopupItemClickHandlers() {
    popupGallery
      .querySelectorAll(".popup-gallery-item img, .popup-gallery-item video")
      .forEach((mediaEl) => {
        ...
        mediaEl.addEventListener("click", (e) => {
          e.stopPropagation();
          const type =
            mediaEl.dataset.lightboxType ||
            (mediaEl.tagName.toLowerCase() === "video" ? "video" : "image");
    ...
    function initPopupCursorDelegation() {
      ...
      document.addEventListener(
        ...
        function (e) {
          ...
          var isInteractive =
            el && el.closest && !!el.closest(interactiveSelector);
